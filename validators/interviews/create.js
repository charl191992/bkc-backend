...
const createInterviewRules = [
  body("application")
    ...
    .custom(async value => {
      ...
      const interview = await Interview.exists({ application: value });
      if (interview)
        throw new Error(
          "Application already have a scheduled interview. You can update the interview details for re-schedule."
        );
